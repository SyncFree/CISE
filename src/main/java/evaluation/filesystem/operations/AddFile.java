...
public class AddFile implements Operation  {
	...
	public BoolExpr precondition(Context ctx) throws Z3Exception {
		 ...
		 Expr name=ctx.mkConst("name1", ctx.mkUninterpretedSort(ctx.mkSymbol("Name")));;
	 	 ...
	 	 Expr precondition=ctx.mkAnd(new BoolExpr[]{(BoolExpr) mustUnique, (BoolExpr) reachable, ctx.mkEq(node, fileId), ctx.mkEq(dir, dirId), ctx.mkEq(fname, name)});
	...
	//	filesystem.FSObject_set= ctx.mkSetAdd(filesystem.FSObject_set,ctx.mkConst(ctx.mkSymbol("dir3"+index), ctx.mkUninterpretedSort(ctx.mkSymbol("FSObject"))));
