	...
	public BoolExpr getInv(Context ctx) throws Z3Exception {
		 ...
		 Expr dirId2=ctx.mkConst("dir3", ctx.mkUninterpretedSort(ctx.mkSymbol("Dir")));
     	 ...
     	 Expr exist= ctx.mkApp(filesystemcrdt.exists, finalArgs)  ;
  		 ...
  		 Expr reachableTuple=filesystemcrdt.Reachability.mkDecl().apply(argreachable);
     	 
     	 Expr reachability=(BoolExpr) ctx.mkSetMembership(reachableTuple, filesystemcrdt.Reachable_set);
    	 ...
    	 Expr exist1= ctx.mkApp(filesystemcrdt.exists, existArgs1)  ;

 	     Expr[] argreachable1= new Expr[3];
 	     argreachable1[0]=dirId2;
 	     argreachable1[1]=dirId1;
 	     argreachable1[2]=exist1;	
 		 Expr reachableTuple1=filesystemcrdt.Reachability.mkDecl().apply(argreachable1);

    	 Expr rechability1=(BoolExpr) ctx.mkSetMembership(reachableTuple1, filesystemcrdt.Reachable_set);
