...
public class MoveDirectoryCRDT2 implements Operation  {
	...
	public MoveDirectoryCRDT2(String name, Context ctx) {
		...
		try {
			...
			father1Id=ctx.mkConst("dir_s2", ctx.mkUninterpretedSort(ctx.mkSymbol("Dir")));
		    Expr[] nodeArgs1 =  new Expr[3];
		    nodeArgs1[0]=father1Id;
		    nodeArgs1[1]=filesystemcrdt.type.getConsts()[0];
		    nodeArgs1[2]=ctx.mkConst("name_s2", ctx.mkUninterpretedSort(ctx.mkSymbol("Name")));
	   	 	
		    index2=rand.nextInt((6 - 0) + 0) + 0;

	   	 	father2Id=ctx.mkConst("dir_d2", ctx.mkUninterpretedSort(ctx.mkSymbol("Dir")));
	   	 	
		    Expr[] nodeArgs2 =  new Expr[3];
		    nodeArgs2[0]=father2Id;
		    nodeArgs2[1]=filesystemcrdt.type.getConsts()[0];
		    nodeArgs2[2]=ctx.mkConst("name_d2", ctx.mkUninterpretedSort(ctx.mkSymbol("Name")));
	   	 
		    index=rand.nextInt((10 - 8) + 0) + 0;;
		    
	   	 	id=ctx.mkConst("dir2", ctx.mkUninterpretedSort(ctx.mkSymbol("Dir")));
	    	dname=ctx.mkConst("name2", ctx.mkUninterpretedSort(ctx.mkSymbol("Name")));;
	    	nodeId=ctx.mkConst("dir2", ctx.mkUninterpretedSort(ctx.mkSymbol("FSObject")));
	   	   	 	
		    Expr[] nodeArgs =  new Expr[3];
		    nodeArgs[0]=ctx.mkConst("dir2", ctx.mkUninterpretedSort(ctx.mkSymbol("FSObject")));;
	...
	public Expr effect(Context ctx) throws Z3Exception {
	    ...
	    argParent[1]=ctx.mkConst("dir2", ctx.mkUninterpretedSort(ctx.mkSymbol("FSObject")));
	    argParent[2]=dname;
	    ...
	    argParent2[1]=ctx.mkConst("dir2", ctx.mkUninterpretedSort(ctx.mkSymbol("FSObject")));
	    argParent2[2]=dname;	
